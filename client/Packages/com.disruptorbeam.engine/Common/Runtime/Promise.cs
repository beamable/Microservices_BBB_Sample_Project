...
{
   // A base class for promise only to reduce things that exist in generic class
   ...
   public class Promise<T> : PromiseBase, ICriticalNotifyCompletion
   {
      ...
      public Promise<TU> Map<TU>(Func<T, TU> callback)
      {
         ...
         Then(value => result.CompleteSuccess(callback(value)))
   ...
   // These are extension methods rather than instance methods to reduce code size
   public static class PromiseExtensions
   {
      ...
      public static Promise<T> RecoverWith<T>(this Promise<T> promise, Func<Exception, Promise<T>> callback)
      {
         ...
         {
            callback(err).Then(value => result.CompleteSuccess(value)).Error(errInner =>
            {
