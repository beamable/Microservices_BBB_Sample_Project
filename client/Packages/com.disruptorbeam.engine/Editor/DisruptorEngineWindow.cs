...
using DisruptorBeam.Editor.Login;
using DisruptorBeam.Editor.Register;
using DisruptorBeam.Editor.Toolbox;
using DisruptorBeam.Modules.AccountManagement;
using DisruptorBeam.Modules.Avatars;
using UnityEngine;
using UnityEditor;
#if UNITY_2018
...
namespace DisruptorBeam.Editor
{
   class DisruptorEngineWindow : EditorWindow
   {
      ...
      private EditorDisruptorEngine _de;
      ...
      public async void OnEnable()
      {
         ...
         _de = await EditorDisruptorEngine.Instance;
         _contentRoot = _windowRoot.Q("de-window-content");

         root.Q<Button>(className: "de-window-logout").clickable.clicked += () =>
         {
            _de.Logout();
            RefreshContent();
         };
      ...
      LoginVisualElement CreateLogin()
      {
         ...
         login.OnLogin += async (email, password) =>
         {
            ...
            {
               login.BlockInput();
               var response = await _de.AuthService.Login(email, password);
