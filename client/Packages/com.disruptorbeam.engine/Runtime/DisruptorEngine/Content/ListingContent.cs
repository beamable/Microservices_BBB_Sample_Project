...
using UnityEngine;
using Promise = Core.Platform.SDK.Promise;
...
namespace DisruptorBeam.Content
{
   ...
   {
      public static Core.Platform.SDK.Promise<Dictionary<string, Sprite>> ResolveAllIcons(this List<OfferObtainCurrency> self)
      {
         List<Core.Platform.SDK.Promise<CurrencyContent>> toContentPromises = self
            .Select(x => x.symbol)
            .Distinct()
         ...
         return Promise.Sequence(toContentPromises)
            .Map(contentSet => contentSet.ToDictionary(
               content => content.Id,
               content => Core.Platform.SDK.PromiseExtensions.ToPromise(content.Icon.LoadAssetAsync().Task))
            ).FlatMap(dict =>
               Promise
