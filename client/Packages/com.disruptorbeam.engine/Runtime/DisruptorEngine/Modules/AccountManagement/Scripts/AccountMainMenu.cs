...

{
...

   {
      ...
      promoPaginationTransform.localPosition = new Vector3(promoPaginationTransform.localPosition.x, -textTransform.rect.height - promoPadding, 0);
...

   {
      ...
      _allUsersPromise = Promise.Sequence(new List<Promise<Unit>> {_mainAccountPromise.ToUnit(), _otherAccountsPromise.ToUnit()}).ToUnit();
...

      {
         ...
         var allAccountPromises = new List<Promise<AccountDisplayItem>>
