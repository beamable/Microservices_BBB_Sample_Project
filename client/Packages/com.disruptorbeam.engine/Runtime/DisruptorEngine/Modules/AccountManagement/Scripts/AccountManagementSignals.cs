...
namespace DisruptorBeam.Modules.AccountManagement
{
   ...
   public class AccountManagementSignals : DeSignalTower
   {
      ...
      {
         DisruptorEngine.Instance.Then(de => de.OnUserChanged -= TriggerUserLoggedIn );
      }
      ...
      {
         WithLoading("Fetching Account...", DisruptorEngine.Instance.FlatMap(de =>
         {
      ...
      public void Login(string email, string password)
      {
         ...
         WithLoading("Logging In...", DisruptorEngine.Instance.FlatMap(de =>
         {
      ...
      public void LoginThirdParty(ThirdPartyLoginArgument argument)
      {
         ...
         {
            return DisruptorEngine.Instance.FlatMap(de =>
            {
      ...
      private Promise<User> GetAccountWithCredentials(IDisruptorEngine de, string email, string password)
      {
         return de.AuthService.Login(email, password, false)
            .FlatMap(token => SetPendingUser(de, token));
      }

      private Promise<User> GetAccountWithCredentials(IDisruptorEngine de, AuthThirdParty thirdParty, string accessToken)
      {
         return de.AuthService.LoginThirdParty(thirdParty, accessToken, false)
            .FlatMap(token => SetPendingUser(de, token));
      }

      private Promise<User> SetPendingUser(IDisruptorEngine de, TokenResponse token)
      {
         _pendingToken = token;
      ...
      private Promise<Unit> LoginToNewUser(IDisruptorEngine de)
      {
         return WithCriticalLoading("New Account...", de.AuthService.CreateUser()
            .FlatMap(de.ApplyToken));
      }

      private Promise<User> AttachEmailToCurrentUser(IDisruptorEngine de, string email, string password)
      {
         return WithCriticalLoading("Loading...", de.AuthService.RegisterDBCredentials(email, password)
            .Then(de.UpdateUserData));
      }

      private Promise<User> AttachThirdPartyToCurrentUser(IDisruptorEngine de, AuthThirdParty thirdParty, string accessToken)
      {
      ...
      private Promise<User> GetExistingAccount(IDisruptorEngine de, UserBundle bundle)
      {
      ...
      {
         return DisruptorEngine.Instance.FlatMap(de => de.AuthService.IsEmailAvailable(email)
