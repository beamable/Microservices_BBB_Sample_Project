...
namespace Core.Platform.SDK.Analytics.Batch {
	...
	public class PersistentBatchManager<T> : BatchManager<T> where T : class, JsonSerializable.ISerializable{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Core.Platform.SDK.Analytics.Batch.PersistentBatchManager`1"/> class.
		/// </summary>
		...
		protected void BackupEventBatch()
		{
			try {
				...
				{
					string batchJson = Core.Serialization.SmallerJSON.Json.Serialize (batchDictionary, pooledBuilder.Builder);
		...
		protected void RestoreEventBatch()
		{
			try {
				...
				{
					var parsedDictionary = Core.Serialization.SmallerJSON.Json.Deserialize(batchJson) as Dictionary<string, object>;
