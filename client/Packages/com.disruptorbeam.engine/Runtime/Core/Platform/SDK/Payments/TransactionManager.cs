...
namespace Core.Platform.SDK.Payments
{
   public class TransactionManager
   {
      ...
      private void BackupPendingPurchases()
      {
         ...
         try
         {
            ...
            {
               string s = Core.Serialization.SmallerJSON.Json.Serialize(dict, pooledBuilder.Builder);
      ...
      private void RestorePendingPurchases()
      {
         ...
         try
         {
            ...
            if (!string.IsNullOrEmpty(encodedPending))
            {
               ...
               {
                  var pending = Core.Serialization.SmallerJSON.Json.Deserialize(decodedPending) as Dictionary<string, object>;
                  if (pending != null)
                  {
      ...
      private void BackupUnfulfilledTransactions()
      {
         ...
         try
         {
            ...
            using (var pooledBuilder = StringBuilderPool.StaticPool.Spawn())
            {
               string s = Core.Serialization.SmallerJSON.Json.Serialize(unfulfilledTransactions, pooledBuilder.Builder);
               string encoded = Convert.ToBase64String(Encoding.UTF8.GetBytes(s));
      ...
      private void RestoreUnfulfilledTransactions()
      {
         ...
         try
         {
            ...
            if (!string.IsNullOrEmpty(encodedUnfulfilled))
            {
               ...
               {
                  var ts = Core.Serialization.SmallerJSON.Json.Deserialize(retrievedUnfulfilled) as Dictionary<string, object>;
                  if (ts != null)
                  {
