...
namespace Core.Service
{
	...
	public static class ApplicationLifetime
	{
		...
		{
			Core.Spew.AppLifetimeLogger.Log("Application Lifetime Detection Online.");
			
         UnityEditor.EditorApplication.playModeStateChanged += (obj) => {
				if (!UnityEditor.EditorApplication.isPlaying)
				{
					Core.Spew.AppLifetimeLogger.Log("Resetting lifetime quit flag to false.");
					isQuitting = false;
				}
			};
		}
	...
	public class ApplicationLifetimeDetector : MonoBehaviour
	{
		...
		{
			Core.Spew.AppLifetimeLogger.Log("Lifetime quit flag set to true.");
			ApplicationLifetime.isQuitting = true;
		}

		void Awake()
		{
			Core.Spew.AppLifetimeLogger.Log("Lifetime quit flag set to false.");
