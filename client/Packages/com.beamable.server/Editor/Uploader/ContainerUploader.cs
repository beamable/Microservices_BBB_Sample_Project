...
public class ContainerUploader
   {
      ...
      public ContainerUploader(EditorDisruptorEngine disruptorEngine, ContainerUploadHarness harness, MicroserviceDescriptor descriptor, string imageId)
      {
         _client = new HttpClient();
         _client.DefaultRequestHeaders.Add("x-ks-clientid", disruptorEngine.Cid);
         _client.DefaultRequestHeaders.Add("x-ks-projectid", disruptorEngine.Pid);
         _client.DefaultRequestHeaders.Add("x-ks-token", disruptorEngine.Token.Token);
         var serviceUniqueName = GetHash($"{disruptorEngine.Cid}_{disruptorEngine.Pid}_{descriptor.Name}").Substring(0, 30);
      ...
      public async Task Upload(string folder)
      {
         ...
         await UploadManifestJson(uploadManifest);
      }
      ...
      private async Task UploadManifestJson(Dictionary<string, object> uploadManifest)
      {
         var manifestJson = Json.Serialize(uploadManifest, new StringBuilder());
         const string reference = DockerTagReference;
         var uri = new Uri($"{_uploadBaseUri}/manifests/{reference}");
