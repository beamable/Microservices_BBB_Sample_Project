   ...
   public class DebugWindow : CommandRunnerWindow
   {
      ...
      private void OnGUI()
      {
         ...
         {
            EditorDisruptorEngine.Instance.Then(de => { de.GetMicroserviceManager().GetCurrentManifest().Then(
               manifest =>
               {
         ...
         {
            EditorDisruptorEngine.Instance.Then(de => { de.GetMicroserviceManager().GetManifests().Then(res =>
            {
         ...
         {
            var wnd = DeployWindow.ShowPopup();
         }
         ...
         foreach (var service in Microservices.Descriptors)
         {
            ...
            if (GUILayout.Button("Upload container"))
            {
               ...
               uploader.UploadContainer(service);
            }
